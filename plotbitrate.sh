...
jq -r '
  [
    "codec",                                "type",                                          "preset",
    "threads",                              "outfile_size",                                  "outfile_options",
    "elapsed_encode_second",                "elapsed_encode_time",                           "compression_ratio_persent",
    "compression_speed",                    "ssim_mean",                                     "harmonic_mean",
    "vmaf_mean",                            "vmaf_harmonic_mean"
  ],
  (.encodes[] |
  [
    .codec,                                 .type,                                           .preset,
    .threads,                               .outfile.size,                                   (.outfile.options | join(" ")),
    .elapsed.encode.second,                 .elapsed.encode.time,                            .results.compression.ratio_persent,
    .results.compression.speed,             .results.vmaf.pooled_metrics.float_ssim.mean,    .results.vmaf.pooled_metrics.float_ssim.harmonic_mean,
    .results.vmaf.pooled_metrics.vmaf.mean, .results.vmaf.pooled_metrics.vmaf.harmonic_mean
  ]) |
  @csv
